...
class BaseScraper(ABC):
    def __init__(self, platform_name: str, headers: Dict[str, str] = None):
    ...
    @retry(
        ...
        retry=retry_if_exception_type((requests.RequestsError, Exception)),
        reraise=True
    )
    def _make_request(self, method: str, url: str, **kwargs):
        """
        Wrapper for making requests with retry logic and impersonation.
        """
        try:
            ...
            logger.debug(f"Making request to {url} with UA: {request_headers.get('User-Agent', 'N/A')}")
